(() => {
  ...
  function renderCommentFileState() {
    ...
    ui.commentFile.value = 'Click to choose comment file';
  }
  ...
  function readFileAsText(file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = () => reject(new Error('Failed to read local comment file'));
  ...
  async function handleCommentFileSelect(event) {
    ...
    try {
      ...
      await appendLog('success', 'Comment file selected and cached', {
        File: file.name,
        Bytes: file.size
      });
    } catch (error) {
  ...
  async function logPeriodicPageRefreshIfNeeded() {
    ...
    await appendLog('info', 'Periodic refresh checkpoint reached. Reopening the page to release X page resources', {
      CompletedRounds: state.loopIndex,
      RefreshEveryRounds: PAGE_REFRESH_EVERY_ROUNDS
    });
  }
  ...
  function renderLogs() {
    ...
    renderLogList(ui.logList, ui.logCount, state.logs, 'No logs yet');
  }
  ...
  function renderTargetLogs() {
    ...
    renderLogList(ui.targetLogList, ui.targetLogCount, state.targetLogs, 'No check logs yet');
  }
  ...
  function createPanel() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        <nav class="xta-tabs" role="tablist" aria-label="Feature tabs">
          <button class="xta-tab is-active" id="xta-tab-assistant-button" type="button" role="tab" aria-selected="true" aria-controls="xta-tab-assistant" data-tab="assistant">Mutual Follow Assistant</button>
          <button class="xta-tab" id="xta-tab-target-check-button" type="button" role="tab" aria-selected="false" aria-controls="xta-tab-target-check" data-tab="target-check">Followed Target Check</button>
        </nav>
          ...
          <div class="xta-controls">
            <section class="xta-card">
              <div class="xta-row xta-settings-row">
                <label class="xta-field">
                  <span>Keyword</span>
                  <input class="xta-input xta-keyword" type="text" value="mutual follow">
                </label>
                <label class="xta-field">
                  <span>Comment file</span>
                  <input class="xta-input xta-comment-file" type="text" readonly value="Click to choose comment file">
                </label>
              </div>
              <input class="xta-file-input" type="file" accept=".txt,text/plain">
              <div class="xta-help">Click the comment file field to choose a local TXT file. It will be remembered automatically.</div>
            </section>
            ...
            <section class="xta-card xta-checks">
              ...
              <label class="xta-check">
                ...
                <span>Dry run, do not actually follow or reply</span>
              </label>
              <label class="xta-check">
                <input class="xta-only-blue" type="checkbox">
                <span>Blue verified only</span>
              </label>
              <div class="xta-row">
                <label class="xta-field">
                  <span>Loop count, 0 for unlimited</span>
            ...
            <section class="xta-card">
              <div class="xta-actions">
                <button class="xta-button primary xta-start" type="button">Start</button>
        ...
        <div class="xta-body xta-tab-panel" id="xta-tab-target-check" role="tabpanel" aria-labelledby="xta-tab-target-check-button" data-tab-panel="target-check" hidden>
          <div class="xta-controls">
            <section class="xta-card xta-target-actions-card">
              <div class="xta-actions">
                <button class="xta-button primary xta-target-start" type="button">Start Check</button>
                <button class="xta-button danger xta-target-stop" type="button" disabled>Stop</button>
              </div>
            </section>

            <section class="xta-card xta-target-options-card">
              <div class="xta-target-option-block">
                <strong class="xta-option-title">Pages to Check</strong>
              ...
              <div class="xta-target-option-block">
                <strong class="xta-option-title">Processing Mode</strong>
                ...
                <div class="xta-help xta-target-note">Unfollow interval is random 5-10 seconds.</div>
              </div>
            </section>

            <section class="xta-card xta-metrics-card">
              <div class="xta-metrics xta-target-metrics">
                <div class="xta-metric"><span>Checked Following</span><strong class="xta-target-checked">0</strong></div>
                <div class="xta-metric"><span>Not Following Me</span><strong class="xta-target-not-followed-by">0</strong></div>
                <div class="xta-metric"><span>Unfollowed</span><strong class="xta-target-unfollowed">0</strong></div>
              </div>
            </section>
          </div>
    ...
    const updatePanelCollapsed = (collapsed) => {
      ...
      ui.toggleButton.innerHTML = collapsed ? xLogoIcon : 'Collapse';
  ...
  function sendPageRequest(type, payload = {}, timeoutMs = 30 * 60 * 1000) {
    ...
    return injectPageScript().then(() => new Promise((resolve, reject) => {
      const timeoutId = window.setTimeout(() => {
        ...
        reject(new Error('Page execution timed out'));
      }, timeoutMs);
  ...
  function parseComments(text) {
    ...
    if (comments.length === 0) {
      throw new Error('No usable comments in comment file');
    }
  ...
  async function loadComments() {
    if (!state.commentFileCache.text) {
      throw new Error('Please choose a local comment file first');
    }
  ...
  async function sleepWithStop(totalSeconds, label) {
    for (let remaining = totalSeconds; remaining > 0; remaining -= 1) {
      ...
      if (
        ...
        setStatus(`${label} ${remaining}s`, 'running');
      }
      await sleep(1000);
    }
  ...
  async function startLoop() {
    ...
    state.loopIndex = 0;
    updateButtons();
    setStatus('Preparing to open live search page', 'running');
  ...
  async function resumePendingRun() {
    ...
    try {
      ...
      if (nextRunAt > Date.now()) {
        ...
        await appendLog('info', 'Continuing to wait for next refresh', {
          RemainingSeconds: remainingSeconds,
          ScheduledTime: new Date(nextRunAt).toLocaleTimeString('en-US', { hour12: false })
        });

        const shouldWait = await sleepWithStop(remainingSeconds, 'Next round countdown');
        if (!shouldWait) {
      ...
      setStatus(`Round ${nextLoopNumber}: waiting for capture`, 'running');
      await appendLog('info', `Round ${nextLoopNumber} started, waiting for real timeline response`, {
        CompletedRounds: state.loopIndex,
        DryRun: options.testMode !== false,
        BlueVerifiedOnly: Boolean(options.onlyBlueVerified),
        Loop: Boolean(options.loopEnabled),
      ...
      await appendLog(actionableCount > 0 ? 'success' : 'warn', `Round ${nextLoopNumber} completed`, {
        TimelineEntries: result.entriesCount,
        MatchedItems: matchedCount,
        ActionableItems: actionableCount,
        CountedThisRound: actionableCount > 0 ? 'Yes' : 'No',
        CompletedRounds: state.loopIndex,
      ...
      if (actionableCount === 0 && !state.stopping) {
        ...
        pendingRun.loopIndex = state.loopIndex;
        await savePendingRun(pendingRun);
        await appendLog('warn', 'No follow or reply actions are needed this round. The loop count is not consumed; retrying after a short wait', {
          MatchedItems: matchedCount,
          ActionableItems: actionableCount,
          ActualWaitSeconds: waitSeconds,
          ScheduledTime: new Date(pendingRun.nextRunAt).toLocaleTimeString('en-US', { hour12: false })
        });

        const shouldRetry = await sleepWithStop(waitSeconds, 'No-action retry countdown');
        if (!shouldRetry) {
          await clearPendingRun();
          setStatus('Stopped', 'idle');
          return;
      ...
      if (!options.loopEnabled || state.stopping || (options.loopCount > 0 && state.loopIndex >= options.loopCount)) {
        await clearPendingRun();
        setStatus(state.stopping ? 'Stopped' : 'Completed', state.stopping ? 'idle' : 'success');
        return;
      }
      ...
      pendingRun.loopIndex = state.loopIndex;
      await savePendingRun(pendingRun);
      await appendLog('info', 'Waiting before reopening live search for the next round', {
        ConfiguredIntervalSeconds: options.intervalSeconds,
        ActualWaitSeconds: waitSeconds,
        ScheduledTime: new Date(pendingRun.nextRunAt).toLocaleTimeString('en-US', { hour12: false })
      });

      const shouldContinue = await sleepWithStop(waitSeconds, 'Next round countdown');
      if (!shouldContinue) {
        await clearPendingRun();
        setStatus('Stopped', 'idle');
        return;
  ...
  async function startTargetCheck() {
    ...
    try {
      ...
      setStatus(result.stopped ? 'Stopped' : 'Check completed', result.stopped ? 'idle' : 'success');
      await appendTargetLog(result.stopped ? 'warn' : 'success', 'Followed target check finished', {
  ...
  async function resetStats() {
    ...
    await saveStats();
    renderStats();
    await appendLog('info', 'Counters reset', {});
  }
