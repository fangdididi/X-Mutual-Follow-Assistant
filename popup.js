...
async function ensureContentScript(tabId) {
  try {
    ...
    // The current tab may not have the content script injected yet.
  }
...
async function refreshPageStatus() {
  ...
  els.pageStatus.textContent = ok ? 'Current x.com tab is ready' : 'Open a signed-in x.com page first';
...
async function showPanel() {
  const tab = await getActiveTab();
  if (!isXTab(tab)) {
    els.pageStatus.textContent = 'Open a signed-in x.com page first';
    return;
  }

  await ensureContentScript(tab.id);
  await sendTabMessage(tab.id, { type: 'XTA_SHOW_PANEL' });
  els.pageStatus.textContent = 'Page panel is visible';
}
